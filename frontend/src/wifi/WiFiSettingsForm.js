...
import { Checkbox, List, ListItem, ListItemText, ListItemAvatar, ListItemSecondaryAction } from '@mui/material';
...
import { PasswordValidator, BlockFormControlLabel, FormActions, FormButton } from '../components';
...
class WiFiSettingsForm extends React.Component {
  ...
  constructor(props) {
    ...
    const { selectedNetwork } = WiFiConnectionContext;
    if (selectedNetwork) {
      const wifiSettings = {
  ...
  deselectNetworkAndLoadData = () => {
    this.WiFiConnectionContext.deselectNetwork();
    this.props.loadData();
  }
  ...
  render() {
    const { selectedNetwork, deselectNetwork } = this.WiFiConnectionContext;
